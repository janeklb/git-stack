...
- Core daily commands should map directly to user intent: create work, inspect stack state, restack history, submit PRs, clean up merged work, and move forward after merges.
- The post-merge `forward` workflow is first-class and should have a dedicated command rather than being hidden behind flags on a broader maintenance command.
- Cleanup is the reconciliation primitive beneath `forward`, but should also stand on its own as a direct command:
  - `forward` handles the strict "current branch merged, move me forward" flow and includes targeted cleanup of the current merged branch as one step in that workflow.
